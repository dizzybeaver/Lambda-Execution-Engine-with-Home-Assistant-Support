...
Description: Zero-abstraction fast paths with LUGS protection + LRU + prewarming
    ...
    def configure(
        ...
        with self._lock:
            ...
            if cache_size is not None and cache_size > 0:
                old_limit = self._cache_size_limit
...
def execute_fast_path(operation_key: str, func: Callable, *args, **kwargs) -> Any:
    ...
    return _fast_path_instance.execute_fast_path(operation_key, func, *args, **kwargs)
...
def get_hot_operations(limit: int = 10) -> list:
    """Get most frequently called operations."""
    return _fast_path_instance.get_hot_operations(limit)


def get_cached_operations() -> list:
    """Get operations in cache."""
    return _fast_path_instance.get_cached_operations()
...
def configure_fast_path(
    ...
    """Configure fast path."""
    return _fast_path_instance.configure(
        enabled, cache_size, warm_threshold, hot_threshold, critical_threshold
    )


def get_fast_path_config() -> Dict[str, Any]:
    """Get configuration."""
    return _fast_path_instance.get_config()


def prewarm_cache(operation_keys: list) -> int:
    """Prewarm cache."""
    return _fast_path_instance.prewarm(operation_keys)


def prewarm_common_operations() -> int:
    """Prewarm common operations."""
    return _fast_path_instance.prewarm_common()


def clear_fast_path_cache() -> None:
    """Clear cache."""
    _fast_path_instance.clear_cache()


def reset_call_counts() -> None:
    """Reset call counts."""
    _fast_path_instance.reset_call_counts()


def reset_stats() -> None:
    """Reset statistics."""
    _fast_path_instance.reset_stats()
...
def fast_path_decorator(operation_name: str):
    """Decorator for fast path execution."""
