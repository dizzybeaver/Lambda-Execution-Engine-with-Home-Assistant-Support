"""
gateway_wrappers.py - Gateway Convenience Wrappers
Version: 2025.10.16.01
Description: Convenience wrapper functions for gateway operations

FIXES APPLIED (2025.10.16):
- BUG #3: Added timeout parameter to websocket_request wrapper for better API discoverability
...
from gateway_interfaces import GatewayInterface
...
def log_error(message: str, **kwargs) -> None:
    """Log error message."""
    execute_operation(GatewayInterface.LOGGING, 'log_error', message=message, **kwargs)
...
def log_operation_success(operation: str, **kwargs) -> None:
    ...
    execute_operation(GatewayInterface.LOGGING, 'log_operation_success', operation=operation, **kwargs)

def log_operation_failure(operation: str, error: str, **kwargs) -> None:
    """Log operation failure."""
    execute_operation(GatewayInterface.LOGGING, 'log_operation_error', operation=operation, error=error, **kwargs)
...
def decrypt_data(encrypted: str, key: Optional[str] = None) -> str:
    """Decrypt data."""
    return execute_operation(GatewayInterface.SECURITY, 'decrypt', encrypted=encrypted, key=key)

def generate_correlation_id() -> str:
    """Generate correlation ID."""
    return execute_operation(GatewayInterface.SECURITY, 'generate_correlation_id')

def validate_string(value: str, min_length: int = 0, max_length: Optional[int] = None) -> bool:
    """Validate string."""
...
def record_metric(metric_name: str, value: float, **kwargs) -> None:
    """Record metric."""
    execute_operation(GatewayInterface.METRICS, 'record', metric_name=metric_name, value=value, **kwargs)

def increment_counter(counter_name: str, amount: int = 1, **kwargs) -> None:
    """Increment counter."""
    execute_operation(GatewayInterface.METRICS, 'increment', counter_name=counter_name, amount=amount, **kwargs)
